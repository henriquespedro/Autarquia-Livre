...
while ($row_search = $load_search->fetchArray(SQLITE3_ASSOC)) {
    ...
    ?>
        if ($("#search_ul").length === 0) {
                $("#search_ul").append('<li id="divider_search" role="presentation" class="divider"></li><li role="presentation"><a style="font-size: 12px" role="menuitem" tabindex="-1" href="javascript:clear_results_search())" >Apagar Resultados</a></li>');
        } 
        
         $("#divider_search").before('<li><a href="javascript:search_<?php echo $row_search['id']; ?>()" style="font-size: 12px" role="menuitem" tabindex="-1" title="<?php echo $row_search['description']; ?>" ><?php echo $row_search['search_name']; ?></a></li>');
